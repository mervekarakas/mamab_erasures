"""
LSAE_ma_ver — Successive Arm Elimination with vertical assignment (legacy AISTATS).

Assigns all pulls of the next arm to the first available agent. If there
are more than one available agent, it chooses the one with smaller delay.
If no arm is left to assign, it plays random arms on the remaining agents
until the assigned arm pulls are complete.
        ...
        for i in range(self.iters):
            ...
            while (counter < self.iters) and np.any(pull_counts[active_arm_inds] < M_i):
                ...
                for j in range(self.m):
                    ...
                    a_learner = assignments[j] if assignments[j] < self.k else self.rng.choice(active_arm_inds)
