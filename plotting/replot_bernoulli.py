#!/usr/bin/env python
"""Plot K=10 Bernoulli reward results — 3-subplot (M=4, 20, 40)."""
import sys, os
...
# Load Bernoulli results
with open(os.path.join(RESULTS_DIR, 'k10_bernoulli_results.pkl'), 'rb') as f:
    saved = pickle.load(f)
...
for subplot_idx, M in enumerate(Ms):
    ax = axes[subplot_idx]
    vars_plot = full_results[(M, 'bernoulli')]
    ...
    ax.set_ylim([0, 13500])
    # Force y-axis to use 1e3 scale so ticks are clean integers (3, 6, 9, 12)
...
fig.tight_layout()
fig_path = os.path.join(RESULTS_DIR, 'k10_bernoulli_jsait.png')
